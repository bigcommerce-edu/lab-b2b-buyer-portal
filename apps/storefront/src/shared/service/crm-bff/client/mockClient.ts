...
import mockCases from './mockCases.json';
import { type CrmGetRequest, type CrmPostRequest } from './types';
...
const logRequest = <PT = void, QT = void>(
  ...
  logInfo('mockCRMRequest', params);
};

const logResponse = <RT>(
  params: any
) => {
  logInfo<RT>('mockCRMResponse', params);
};

interface MockRequestParams<PT = void, QT = void> extends CrmGetRequest<QT> {
  method: "POST" | "GET";
  payload?: PT;
}
...
const mockRequest = async<RT, PT = void, QT = void>(
  ...
  switch (path) {
    case 'storefrontToken':
      ...
      const crmStorefrontToken = Array.from({ length: 32 }, () => Math.floor(Math.random() * 16).toString(16)).join('');

      const jwt = await new jose.SignJWT({
        token: crmStorefrontToken,
        companyId: '123456',
    ...
    case 'company/orderCases':
      ...
      logResponse<RT>(casesResponse);
      return casesResponse as RT;
