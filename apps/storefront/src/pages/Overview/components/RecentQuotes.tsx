  ...
  const quoteColumns = [
    ...
    },
    {
      ...
      render: (item: OverviewQuote) => {
        return `${Number(item.status) !== 0 ? displayFormat(Number(item.createdAt)) : item.createdAt}`;
      },
    },
    {
      key: 'updatedAt',
      ...
      render: (item: OverviewQuote) => {
        return `${Number(item.status) !== 0 ? displayFormat(Number(item.updatedAt)) : item.updatedAt}`;
      },
    },
    {
      key: 'totalAmount',
